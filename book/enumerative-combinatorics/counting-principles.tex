...
\index{multiplication principle}
\index{rule of product}
Let $n \ge 1$ and $m_1,m_2,\dots,m_n \in \mathbb{N}$. Suppose for each $1 \le i \le n$ that we are given finite sets $X^{(i)}_{a_1,\dots,a_{i-1}}$ with $|X^{(i)}_{a_1,\dots,a_{i-1}}| = m_i$, where $a_j \in X^{(i)}_{a_1,\dots,a_{j-1}}$ for each $j<i$. Define
...
\item (\textbf{Induction step}) Fix $n \ge 1$ and assume that the theorem statement is true for this value of $n$.
...
\item Since our sets are finite, so will $\mathcal{A}$ be; so we will only ever partition our sets into finitely many pieces. That is, all of our partitions will take form $\{ A_1, A_2, \dots, A_n \}$ for some $n \in \mathbb{N}$.
\end{itemize}
...
Let $X$ be a set and let $\{ A_1, \dots, A_n \}$ be a partition of $X$ for some $n \in \mathbb{N}$, such that each set $A_i$ is finite. Then $X$ is finite, and
\[ |X| = |A_1| + |A_2| + \cdots + |A_n| \]
\end{theorem}
...
Let $X$ be a finite set. In order to compute $|X|$, it suffices to find a partition $A_1, A_2, \dots, A_n$ of $X$; it then follows that $|X| = \sum_{k=1}^n |A_i|$.
\end{strategy}
...
\begin{itemize}
\item An element of $E$ must be a subset of $\{0,2,4,6\}$. By \Cref{exNumSubsetsOfFiniteSet} there are $2^4=16$ such subsets. Thus the number of \textit{inhabited} subsets of $[7]$ containing only odd numbers is $15$, since we must exclude the empty set. That is, $|O|=15$.
\item An element of $O$ must be a subset of $\{1,3,5\}$. Again by \Cref{exNumSubsetsOfFiniteSet} there are $2^3=8$ such subsets. Hence there are $7$ inhabited subsets of $[7]$ containing only even numbers. That is, $|E|=7$.
\end{itemize}
