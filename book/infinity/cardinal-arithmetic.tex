...
\begin{cproof}
There is a bijection $i : \mathcal{P}(X) \to [2]^{X}$ defined for all $U \subseteq X$ by letting $i(U) = \chi_U \in [2]^X$, where $\chi_U : X \to [2]$ is the characteristic function of $U$ (see \Cref{defCharacteristicFunction}). Note that $i$ is a bijection by \Cref{thmCharacteristicFunctionsCharacteriseSubsets}.
...
It is worth pointing out that the axiom of choice (\Cref{axChoice}) says precisely that the cartesian product of every family of non-empty sets is non-empty.
