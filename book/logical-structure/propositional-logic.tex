...
\begin{center} \vspace{-10pt} \large \gbus{\textbf{fish n chips}}{\textbf{mac n cheese}} \end{center}
\vspace{-10pt} If you forget whether it's $\wedge$ or $\vee$ that means `and', just write it in place of the `n' in `\gbus{fish n chips}{mac n cheese}':
\begin{center} \vspace{-10pt} \gbus{fish $\wedge$ chips}{mac $\wedge$ cheese} \qquad \qquad \gbus{fish $\vee$ chips}{mac $\vee$ cheese} \end{center}
\vspace{-10pt} Clearly the first looks more correct, so $\wedge$ means `and'. This mnemonic can be modified to accommodate a wide variety of dietary restrictions; for instance `quinoa n kale'.
\end{aside}
...
\todo{Rewrite the next few examples to remove references to complex numbers and polynomials}
