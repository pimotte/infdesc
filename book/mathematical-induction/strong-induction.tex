...
\end{itemize}
By the well-ordering principle, $X$ has a minimal element $r \in X$, and by definition of $X$ there exists $q \in \mathbb{Z}$ such that $a=qb+r$. Since $r \in X$, we have $r \in \mathbb{N}$, so that $r \ge 0$.

It remains to prove that $r<b$.

Towards a contradiction, assume that $r \ge b$. To obtain a contradiction, will proceed by infinite descent. Define
\[ Y = \{ n \in X \mid n \ge b \} \]
Note that $Y$ is inhabited since $r \in Y$, and $Y \subseteq \mathbb{N}$ since $Y \subseteq X$ and $X \subseteq \mathbb{N}$.
...
We have proved that, for all $n \in Y$, we have $n-b \in Y$ and $n-b < n$, and therefore we have infinite descent in $Y$. This is a contradiction.

Therefore, our assumption that $r \ge b$ is false, and so it must in fact be the case that $r<b$, as required.
\end{example}
