...
% \begin{convention}
% When discussing functions $f : D \to \mathbb{R}$ in this section, we assume that the domain $D$ is an non-empty interval in $\mathbb{R}$. Thus either $D = \mathbb{R}$, or $D$ is one of the subsets of the kind defined in \Cref{defIntervals}.
% \end{convention}
...
% \fixlistskip
% \begin{itemize}
% \item ($\Rightarrow$) Suppose $f$ is continuous, and let $U \subseteq \mathbb{R}$ be open. If $f^{-1}[U] = \varnothing$, then we can take $V = \varnothing$. So assume that $f^{-1}[(a,b)]$ is non-empty and fix $p \in f^{-1}[U]$.
