...
\dots{} \xtremph{if $r \ge b$}, then $r-b \in \mathbb{N}$ and $a = qb+r = (q+1)b + (r-b)$, which implies that $r-b \in X$; but $r-b<r$ since $b>0$, so \xtremph{this contradicts minimality of $r$}. \xtremph{Therefore $r<b$} \dots{}
\end{extract}
...
We need to prove \xtremph{$f(k) \in X$ for all $k \in [2n+1]$}. Well \xtremph{given $k \in [2n+1]$}, we have $0 \le k \le 2n$, and so
...
Nonetheless, we used the construction in \Cref{vcbWlog} at other times, as illustrated in the next extract:

\begin{extract}[\xtrsource{\Cref{thmDivisionTheorem}}]
\label{xtrWlogExampleTwo}
\xtremph{We may assume that $r_0 \le r_1$---otherwise, swap the roles of $(q_0,r_0)$ and $(q_1,r_1)$ in what follows.}
\end{extract}
