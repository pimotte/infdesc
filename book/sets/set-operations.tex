...
However, there are contexts where the sets of interest are all subsets of some fixed set. For example, we might want to study subsets of a fixed set $X$ in the context of order theory (as in \Cref{secOrderRelations}). In these contexts, we may use the \textit{notation} for absolute complements, but it will formally be the relative complement within the larger set---that is, $A^{\mathsf{c}} = \Omega \setminus A$ in the former context, and $A^{\mathsf{c}} = X \setminus A$ in the latter.
